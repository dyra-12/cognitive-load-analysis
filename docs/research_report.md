...
**Key Achievement:** A tuned Random Forest classifier achieved **96% accuracy (LOUO), with F1 = 0.82 and ROC-AUC = 0.95**, with SHAP analysis revealing interpretable cognitive-load signatures.
...
- HCI and UX researchers studying cognitive load
- ML practitioners interested in behavior-based inference
- Designers exploring adaptive interfaces
- Students learning explainable AI in applied settings
...
.
├── data/
│   ├── raw/                      # Original anonymized telemetry logs (subject to ethics constraints)
...
Repeated-measures ANOVA confirmed statistically significant differences in reported workload across tasks:

**F(2,48) = 87.3, p < .001, η² = 0.54**
...
| **Precision** | 0.68 |
| **Recall** | 0.66 |
| **F1-score (mean LOUO)** | 0.82 |
| **ROC-AUC** | 0.95 |

**Note:** The reported F1-score reflects the mean across LOUO folds; precision and recall vary across participants due to the small per-fold test size.
...
- ✔️ **Ethics review completed** - IRB-exempt / minimal-risk study
